...
def main():
    ...
    parser.add_argument("--data_paths_json", default='data/NIH/NIH_regist_pair.json')
    ...
    for i in range(100):
        ...
        try:
            Wf0, Hf0 = np.asarray(data[i]["fixed_size"], dtype=np.float64)
            Wm0, Hm0 = np.asarray(data[i]["moving_size"], dtype=np.float64)
            ...
            print(f"Error loading data for pair {i}: {e}")
            c_val = None
